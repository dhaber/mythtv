...
#Find Kids shows that we should re-record so we get a better version
...
def goodRec(x):
	return x.recgroup == 'Kids' 
#	return x.subtitle is not None and x.title not in titlesIgnore and (x.recstatus == Program.rsWillRecord or x.recstatus == Program.rsConflict) 
...
for p in sorted(pending, key=pendFun):
	print "%s %s %s" % (p.title, p.subtitle, p.recstatus)
